\section{Age Group Classification Performance on Texts Excluding Stopwords}
...
\begin{table*}[h]
    ...
    \bottomrule
    \end{tabular}
    \caption{Discourse dataset, \textbf{excluding} stopwords. Test set results averaged over 5 random initializations. Format: \textit{average metric (standard error)}. Values in \textbf{bold} are the highest in the column; in \textcolor{blue}{blue}, the second highest. *: $F_1$ is actually $0/0$.}
    % \san{If we agree with the abbreviations used in this table, we could use them also in Table 3}}
    \label{tab:blog_classification_excl_stopwords}
\end{table*}
...
\begin{table*}[h]
    ...
    \end{tabular}
    % }
    \caption{Dialogue dataset, \textbf{excluding} stopwords. Test set results averaged over 5 random initializations. Format: \textit{average metric (standard error)}. Values in \textbf{bold} are the highest in the column; in \textcolor{blue}{blue}, the second highest.}
    \label{tab:bnc_classification}
\end{table*}


% \section{Prompts used for controlled dialogue response generation}
...
% \section{Controlled Text Generation Results for Unprompted Setup \len{Redundant?}}
