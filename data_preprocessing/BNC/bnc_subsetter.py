...
def main(args):
    ...
    keys = ['speaker_id', "conv_id", 'turn', 'text', 'length', 'label']
        ...
        if conv.n_speakers == N:
            ...
            if same_group1:
                for u in conv.utterances:
                    ...
                    df.loc[0 if pd.isnull(df.index.max()) else df.index.max() + 1] = [u.speaker_id] + [conv_id] + \
    ...
    df.to_csv(
        args.save_dir +
        f'bnc_subset_{args.label1}_vs_{args.label2}_nfiles_{args.no_files}.csv',
        index=False
    )
