...
#SBATCH --job-name=ctg_train_discrim_gpt2_tryout
...
#SBATCH -o /home/lennertj/code/msc-ai-thesis/SLURM/output/ctg_train_discrim_gpt2_tryout.%A.out ## this is where the terminal output is printed to. %j is root job number, %a array number. try %j_%a ipv %A (job id)
...
do
  python plug_play/run_pplm_discrim_train.py \
         --dataset "generic" \
         --dataset_fp "data/bnc/bnc_rb_full_generic_pplm.txt" \
         ...
         --log_interval 10 \
         --save_model \
         --output_fp "plug_play/discriminators/"
done
