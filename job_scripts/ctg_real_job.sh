...
#SBATCH --job-name=scripted_dialogue_generation_hobby_turnwise_YOUNG_utterance_classification
...
#SBATCH -o /home/lennertj/code/msc-ai-thesis/SLURM/output/scripted_dialogue_generation_hobby_turnwise_YOUNG_utterance_classification.%A.out ## this is where the terminal output is printed to. %j is root job number, %a array number. try %j_%a ipv %A (job id)
...
# for discrim-based
for prompt in "${young_utts[@]}"
do
  echo "NEW PROMPT ABI"
  python plug_play/prompt_classifier.py --std --age_cat 0 --prompt "$prompt"
done