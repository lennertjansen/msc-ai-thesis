...
#SBATCH --job-name=ctg_bow_old_fb_pct_80_ws
...
#SBATCH -o /home/lennertj/code/msc-ai-thesis/SLURM/output/ctg_bow_old_fb_pct_80_ws.%A.out ## this is where the terminal output is printed to. %j is root job number, %a array number. try %j_%a ipv %A (job id)
...
do
  ...
  do
    python plug_play/run_pplm.py \
           ...
           --num_samples 30 \
           --bag_of_words 'plug_play/wordlists/bnc_old_mcwu_ws_pct_80.txt' \
           --length $length \
           --seed $seed \
           --sample \
           --class_label 1 \
