...
#SBATCH --job-name=scripted_dialogue_generation_food_turnwise_young_utterance_classification
...
#SBATCH -o /home/lennertj/code/msc-ai-thesis/SLURM/output/scripted_dialogue_generation_food_turnwise_young_utterance_classification.%A.out ## this is where the terminal output is printed to. %j is root job number, %a array number. try %j_%a ipv %A (job id)
...
do
  echo "NEW PROMPT ABI"
  python plug_play/prompt_classifier.py --std --prompt "$prompt"
done