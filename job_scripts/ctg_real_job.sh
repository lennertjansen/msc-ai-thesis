...
#SBATCH --job-name=ctg_old_bow_fb
...
#SBATCH -o /home/lennertj/code/msc-ai-thesis/SLURM/output/ctg_old_bow_fb.%A.out ## this is where the terminal output is printed to. %j is root job number, %a array number. try %j_%a ipv %A (job id)
...
do
  ...
  do
    python plug_play/run_pplm.py \
           ...
           --num_samples 30 \
           --bag_of_words 'plug_play/wordlists/bnc_old_mcwu.txt' \
           --length $length \
           --seed $seed \
           --sample \
           --class_label 1 \
